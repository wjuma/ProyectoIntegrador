...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Function CARGA_TH_1_G2
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "P54G4_OLTP_HR"."CARGA_TH_1_G2" (VANIO INT)  --g1
return number
is 
    cursor c_th_ubic is 
...
--select * from d_tiempos
    INSERT INTO tabla_hechos_2(codanio, id_d_provincia, id_d_grup,
    total_impuestos, total_alquileres, total_iva_cobrado, total_iva_pagado,total_insumos,total_gastos_operacion)
    select anio_d,r_ubic.id_d_provincia,null,
    SUM(t.totimp), SUM(t.totalq),
    SUM(t.imp_ivacob),sum(t.imp_ivapag),
    sum(t.totinsum),sum(t.totgadop)
                   ...
                   INSERT INTO tabla_hechos_2(codanio, id_d_provincia, id_d_grup,
                    ...
                    OPEN c_th_SERV;
                        ...
                        WHILE c_th_SERV%FOUND LOOP 
                           ...
                           INSERT INTO tabla_hechos_2(codanio, id_d_provincia, id_d_grup,id_servicio,
                            total_impuestos, total_alquileres, total_iva_cobrado, total_iva_pagado,total_insumos,total_gastos_operacion)
                            ...
                            group by anio_d,r_ubic.id_d_provincia,r_grup_ser.id_grup_serv,r_servicio.id_ser_1;
                            commit;
